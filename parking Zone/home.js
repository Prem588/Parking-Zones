...
function calculatePayment(entryDate) {
    const todayDate =entryDate =String(entryDate).split("/")[0];
    var cost =((todayDate -new Date().getDate()+1) *100);
    return cost;
}
...
function renderTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        newRow.insertCell(3).textContent = item.entryDate;
        newRow.insertCell(4).textContent ='Rs.'+ calculatePayment(item.entryDate);
...
document.getElementById('saveButton').addEventListener('click', function() {
    ...
    const entryDate = dates.getDate() + "/" + (dates.getMonth() + 1) + "/" + dates.getFullYear();

    if (!name || !vehicleName || !vehicleNumber || !entryDate) {
    ...
    const newEntry = { name, vehicleName, vehicleNumber, entryDate, parkingSpot };
